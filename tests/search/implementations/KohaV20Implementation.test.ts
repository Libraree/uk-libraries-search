...
const folder = `${__dirname}/../data/kohaV20`;
const services = new Services();

test('Gets Libraries', async () => {
    const service = 'St Helens';
    const client = new ProxymanHttpClient(`${folder}/sthelens-libraries.proxymanlogv2`);
    const impl = new KohaV20Implementation(client);
...
test('Gets Books', async () => {
    const service = 'St Helens';
    const client = new ProxymanHttpClient(`${folder}/sthelens-search.proxymanlogv2`);
    const impl = new KohaV20Implementation(client);
