...
const folder = `${__dirname}/../data/iguana`;
const services = new Services();

test('Gets Libraries', async () => {
    const service = 'Sir Benfro - Pembrokeshire';
    const client = new ProxymanHttpClient(`${folder}/pembs-libraries.proxymanlogv2`);
    const impl = new IguanaImplementation(client);
...
test('Gets Books', async () => {
    const service = 'Sir Benfro - Pembrokeshire';
    const client = new ProxymanHttpClient(`${folder}/pembs-search.proxymanlogv2`);
    const impl = new IguanaImplementation(client);
